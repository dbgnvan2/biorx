"""
PsyArXiv (OSF) search adapter.
Psychology and behavioral science preprints.
...
class PsyArxivAdapter:
    ...
    def search(
        ...
        """
        Fetch recent PsyArXiv preprints.

        The OSF preprints endpoint does not support server-side full-text search
        in the same way as Lucene. We fetch by date range and provider, then
        rely on client-side _filter_papers() for text matching.

        Args:
            query:       Plain-text keywords (used if the OSF endpoint accepts q=).
            page:        1-based page number.
            page_size:   Results per page.
            filter_dict: Original filter dict (used to extract date range).

        Returns:
            List of raw OSF preprint data dicts.
        """
        ...
        # OSF supports a basic `filter[title]` for title search — use if we have a query
        if query:
            # Only use the first keyword to avoid over-constraining
        ...
        try:
            ...
            raise SourceUnavailableError(f"PsyArXiv (OSF) unreachable: {e}") from e

        if resp.status_code == 429:
            raise RateLimitedError("PsyArXiv rate limit hit")
        if not resp.ok:
            raise SourceUnavailableError(f"PsyArXiv returned {resp.status_code}")

        data = resp.json()
        results = data.get("data", [])
        logger.debug(
            "PsyArXiv: query=%r page=%s returned=%s",
    ...
    def normalize(self, raw: RawRecord) -> CanonicalRecord:
        ...
        # URLs
        source_url = links.get("html", "") or f"https://osf.io/preprints/psyarxiv/{osf_id}/"
        ...
        return CanonicalRecord(
            ...
            oa_status="open",  # PsyArXiv is open access
            subjects=subjects[:5],
